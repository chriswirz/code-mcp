...
func (s *Server) registerMarkdownTools() {
	...
	s.RegisterTool(Tool{
		...
		args.Path = actual
		original, err := ws.ReadFile(actual)
		if err != nil {
		...
		if formatted == original {
			return withNote(toolResult(fmt.Sprintf("%s is already one sentence per line", name)), note), nil
		}

		before := strings.Count(original, "\n") + 1
		after := strings.Count(formatted, "\n") + 1
		if args.DryRun {
			return withNote(toolResult(fmt.Sprintf("Dry run: nothing was written.\n%s: %d lines would become %d\n%s",
				name, before, after, changedHunk(name, original, formatted))), note), nil
		}
		if _, err := ws.WriteFile(args.Path, formatted); err != nil {
			return toolError("%v", err), nil
		}
		return withNote(toolResult(fmt.Sprintf("%s: %d lines became %d\n%s",
			name, before, after, changedHunk(name, original, formatted))), note), nil
	})
}
