...
func (s *Server) registerLineEndingTools(cfg WorkspaceConfig) {
	...
	s.RegisterTool(Tool{
		...
		// The file scope names one file, so it gets the same correction the
		// other file tools do: a path that is not there but matches exactly
		// one file in the workspace means that file.
		note := ""
		if scope == scopeFile {
			actual, corrected, resolveErr := ws.ResolveForRead(target)
			if resolveErr != nil {
				return pathToolError(ws, target, resolveErr), nil
			}
			target, note = actual, corrected
		}
		...
		if err != nil {
			return pathToolError(ws, target, err), nil
		}
		...
		if plan.Changed == 0 {
			...
			plan.trim()
			return withNote(toolResultJSON(plan), note), nil
		}
