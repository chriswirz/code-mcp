	...
	s.RegisterTool(Tool{
		...
		search := func(path string) bool {
			...
			for i, line := range lines {
				...
				if args.FilesOnly {
					// One hit settles the question for this file. The limit
					// counts files here, since matches are not being returned.
					if len(files) >= limit {
		...
		if args.FilesOnly {
			...
			if result.Truncated {
				text += fmt.Sprintf("\n\n(stopped at %d files)", limit)
			}
			return &CallToolResult{
				Content: textContent(text),
				StructuredContent: map[string]any{
					"pattern": args.Pattern, "files": names, "truncated": result.Truncated,
				},
			}, nil
		}
		return &CallToolResult{
