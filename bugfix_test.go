...
// TestWindowsShellLineWithBackslashIsNotMangled guards a regression in how a
// shell line reaches cmd.exe: Go's exec package escapes the "/c" argument
// using the standard CommandLineToArgvW rules, but cmd.exe re-reads the raw
// command line itself and only strips one matching pair of outer quotes -
// it does not undo the backslash-doubling Go adds before a quote. A line
// containing a quoted, backslash-terminated path (`dir "C:\Program Files\"`)
// or even a bare trailing backslash (`dir C:\`) used to come out corrupted
// and fail with "The filename, directory name, or volume label syntax is
// incorrect", even though both are ordinary things to type on Windows.
func TestWindowsShellLineWithBackslashIsNotMangled(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("cmd.exe's /c parsing quirk is Windows-only")
	}
	dir := t.TempDir()
	...
	for _, line := range []string{
		...
		res, err := runShellSudo(context.Background(), nil, line, dir, nil, 0)
		if err != nil {
			t.Fatalf("runShellSudo(%q): %v", line, err)
		}
		if res.ExitCode != 0 {
			t.Errorf("runShellSudo(%q) exit code = %d, stderr = %s", line, res.ExitCode, res.Stderr)
		}
	}
}
