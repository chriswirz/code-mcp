...
func (s *Server) registerGitHubTools(cfg GitHubConfig) {
	...
	gh := func(ctx context.Context, args ...string) (*CallToolResult, *RPCError) {
		res, err := runExec(ctx, cfg.GhPath, repoArgs(args), s.workspace().Root, ghTimeout)
		if err != nil {
	...
	s.RegisterTool(Tool{
		...
		res, err := runExec(ctx, cfg.GhPath, repoArgs([]string{"run", "watch", args.RunID, "--exit-status"}), s.workspace().Root, 45*time.Minute)
		if err != nil {
	...
	s.RegisterTool(Tool{
		...
		if args.Name == "" {
			dir, err := s.workspace().Resolve(".github/workflows")
			if err != nil {
				return toolError("%v", err), nil
			}
		...
		content, err := s.workspace().ReadFile(".github/workflows/" + strings.TrimPrefix(args.Name, ".github/workflows/"))
		if err != nil {
			return toolError("%v", err), nil
		}
