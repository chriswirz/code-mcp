...
func (s *Server) registerEditTools() {
	...
	s.RegisterTool(Tool{
		...
		for i, op := range args.Edits {
			...
			if resolveErr != nil {
				return prefixToolError(pathToolError(ws, op.Path, resolveErr), fmt.Sprintf("edits[%d]", i)), nil
			}
			op.Path = actual
			...
			if _, seen := staged[key]; !seen {
				...
				if err != nil {
					return prefixToolError(pathToolError(ws, actual, err), fmt.Sprintf("edits[%d]", i)), nil
				}
...
func missingReplacementError(op editOp) error {
	...
	return fmt.Errorf("the edit for %s has no new_string, so it would delete %q rather than change it. "+
		"Pass the replacement in new_string, or an empty string if deleting is what you meant",
		op.Path, truncateForError(firstNonEmpty(op.OldString, op.OldText, op.OldTextSnake)))
}
