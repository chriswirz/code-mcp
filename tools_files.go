...
func (s *Server) registerFileTools() {
	...
	s.RegisterTool(Tool{
		...
		if args.StartLine <= 0 && args.EndLine <= 0 {
			return withNote(toolResult(content), note), nil
		}
		// A file ending in a newline splits with a trailing empty element,
		// which is not a line: counting it makes the file look one line
		// longer than it is and hands back a spurious blank at the end of a
		// range.
		...
		if start > len(lines) {
			return toolError("%s has %d lines, start_line %d is past the end", actual, len(lines), start), nil
		}
		// The slice keeps a final newline only if the file had one and the
		// range runs to its end, so the text reads like the file does.
		return withNote(toolResult(joinLines(lines[start-1:end], finalNewline && end == len(lines))), note), nil
	})
	...
	s.RegisterTool(Tool{
		...
		return withNote(toolResult(fmt.Sprintf("Replaced %d occurrence(s) in %s%s\n%s",
			res.Count, name, adjustedNote, changedHunk(name, content, res.Content))), note), nil
	})
