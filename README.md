...
Console commands: `/help`, `/reload`, `/connect [name]`, `/tools [filter]`, `/models`, `/model <name|id>`, `/thinking [on|off]`, `/stats`, `/save [path]`, `/summary`, `/savesummary [path]`, `/compress`, `/clear`, `/permissions`, `/exit`.

### Saving and compressing the conversation
...
`/savesummary [path]` does the same and writes the note to a text file instead (same path rules as `/save`, a `.md` file by default).
