...
Usage:
  codemcp [flags]
  codemcp agent [agent flags]   interactive console agent (codemcp agent --help)
...
Flags:
  ...
  -u, --url <url>          full URL clients connect to; its host and port are
      ...
      --update             download the latest release, verify it against the
                           release checksums, and replace this binary with it
      --update-version <tag>
                           update to that release tag (0.1.NNNN) instead of the
                           latest, which also downgrades
...
func parseFlags(args []string) (*options, error) {
	...
	str(&opts.updateVersion, "", "update-version")
...
func run(args []string) error {
	...
	case opts.exampleConfig:
		...
		return nil
	case opts.update || opts.updateVersion != "":
		return selfUpdate(os.Stderr, opts.updateVersion)
	}
	...
	// The REST face of the tool set, served on the same listener as the MCP
	// endpoint. It needs one, so it is an HTTP-transport feature only.
	if cfg.OpenAPI.Enabled {
		if cfg.Server.Transport == "http" {
			srv.api = newOpenAPIServer(srv, cfg)
		} else {
			logger.Printf("openapi.enabled is set, but the OpenAPI tool server is served over HTTP " +
				"and this server is on stdio; set server.transport to \"http\" to use it")
		}
	}
	...
	// Background processes belong to this session; killing them on the way out
	// is better than orphaning a dev server nothing is tracking any more.
...
func printBanner(w *os.File, cfg *Config, srv *Server, db *DB) {
	...
	if cfg.Server.Transport == "stdio" {
		...
		// The REST face is easy to forget is there, and its URL is not the
		// endpoint URL, so the banner names it outright.
		if srv.api != nil && len(endpoints) > 0 {
			fmt.Fprintf(w, "  OpenAPI tool server: %s%s/openapi.json\n",
				strings.TrimSuffix(endpoints[0].URL, endpoints[0].Path), cfg.OpenAPI.Path)
		}
