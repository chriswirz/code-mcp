...
func (s *Server) registerCommandTools(commands []CommandConfig) {
	kept := make([]CommandConfig, 0, len(commands))
	for _, cmd := range commands {
		// A command that runs git is not registered at all while git is
		// disabled: an unlisted tool cannot be called by mistake, and the
		// configuration reads the way the operator wrote it.
		if s.checkGitAllowed(commandLine(cmd, cmd.DefaultArgs)) != nil {
			if s.logger != nil {
				s.logger.Printf("commands.%s: not registered, it runs git and git.enabled is false", cmd.Name)
			}
			// Recorded rather than merely dropped, so calling it says why
			// instead of coming back as an unknown name.
			s.markUnavailable(cmd.Name,
				fmt.Sprintf("it runs git (%s) and git.enabled is false", commandLine(cmd, cmd.DefaultArgs)),
				"The other project commands still work; see project_commands.")
			continue
		}
...
func commandTool(cmd CommandConfig) Tool {
	...
	if cmd.Background {
		props["wait_seconds"] = prop("integer",
	...
	if cmd.Background {
		desc += "\n\nStarts in the background and returns a process id immediately rather than waiting: " +
			"poll it with check_process_id, and end it early with stop_process."
	}
	return Tool{
		Name:        cmd.Name,
		Title:       cmd.Name,
...
func (s *Server) commandHandler(cmd CommandConfig) ToolHandler {
	return func(ctx context.Context, raw json.RawMessage) (*CallToolResult, *RPCError) {
		...
		line := commandLine(cmd, args.Args)
		if blocked := s.checkGitAllowed(line); blocked != nil {
			return blocked, nil
		}
		timeout := time.Duration(cmd.TimeoutSeconds) * time.Second
		if cmd.Background {
			return s.startProcessResult(ctx, line, dir, cmd.Env, timeout, args.WaitSeconds), nil
		}
		res, err := runShellSudo(ctx, s.sudoAgentRef(), line, dir, cmd.Env, timeout)
		if err != nil {
...
func (s *Server) registerCommandIndex(commands []CommandConfig) {
	...
	s.RegisterTool(Tool{
		...
		Description: "List the build, test, lint and other commands this project defines in its config.json. Each one is also callable as a tool of the same name. A command marked background starts as a process and returns an id to poll with check_process_id.",
...
func (s *Server) registerShellTool() {
	...
	s.RegisterTool(Tool{
		...
		Description: fmt.Sprintf(
			"Run an arbitrary shell command in the workspace. Use it only when no dedicated tool covers the job: "+
				"this server's own tools are portable, confined to the workspace, and tracked by rollback, and a shell "+
				"command is none of those. Read with read_file or read_files rather than cat or type; list a directory with "+
				"list_directory rather than dir or ls; search with grep_files, "+
				"find_files, find_references or file_outline rather than grep or findstr; change files with edit_file, edit_lines, "+
				"insert_text, multi_edit, remove_sections or replace_in_files rather than sed or echo redirection; use the git_* "+
				"tools rather than git; and check a change with diagnostics or the named project commands (see project_commands).\n\n"+
		...
		if blocked := s.checkGitAllowed(args.Command); blocked != nil {
			return blocked, nil
		}
		...
		result := res.asToolResult()
		if invokesListDir(args.Command) {
			result = withNamingWarning(result, listDirWarning)
		}
		return result, nil
	})
}
