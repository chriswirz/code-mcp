...
func newTestServer(t *testing.T) (*Server, string) {
	...
	cfg.Commands = []CommandConfig{
		...
		{Name: "serve", Description: "Serve it.", Command: echoLoop(t), Background: true},
	}
...
// TestUnknownToolIsAnErrorResult: a name this server does not have comes back
// as a tool error the model can act on, not as a JSON-RPC error, which reads
// to a model like the transport failed and invites the same call again.
func TestUnknownToolIsAnErrorResult(t *testing.T) {
	s, _ := newTestServer(t)
	res, rpcErr := s.Handle(context.Background(), &Request{
		JSONRPC: "2.0", ID: json.RawMessage(`1`), Method: "tools/call",
		Params: withMeta(t, map[string]any{"name": "no_such_tool"}),
	})
	if rpcErr != nil {
		t.Fatalf("want a result, got the protocol error %v", rpcErr)
	}
	result, ok := res.(*CallToolResult)
	if !ok || !result.IsError {
		t.Fatalf("want an error result, got %#v", res)
	}
	if text := result.Content[0].Text; !strings.Contains(text, "tools/list") {
		t.Errorf("the answer should point at the tool list: %s", text)
	}
}
...
func TestSystemStatsTool(t *testing.T) {
	s, root := newTestServer(t)
	got := call(t, s, "tools/call", map[string]any{"name": "system_stats"})
	if got["isError"] == true {
		t.Fatalf("system_stats failed: %v", got)
	}

	structured, _ := got["structuredContent"].(map[string]any)
	cpu, _ := structured["cpu"].(map[string]any)
	if cpu == nil {
		t.Fatal("no cpu section in structured content")
	}
	if logical, _ := cpu["logical_cores"].(float64); logical <= 0 {
		t.Errorf("logical_cores = %v, want > 0", cpu["logical_cores"])
	}

	disk, _ := structured["disk"].(map[string]any)
	if disk == nil {
		t.Fatal("no disk section in structured content")
	}
	if disk["path"] != root {
		t.Errorf("disk.path = %v, want %q (the workspace root)", disk["path"], root)
	}
	if total, _ := disk["total_bytes"].(float64); total <= 0 {
		t.Errorf("disk.total_bytes = %v, want > 0 for a real directory", disk["total_bytes"])
	}

	content, _ := got["content"].([]any)
	block, _ := content[0].(map[string]any)
	text, _ := block["text"].(string)
	if !strings.Contains(text, "CPU") || !strings.Contains(text, "Disk") {
		t.Errorf("the text summary is missing expected sections: %q", text)
	}
}
