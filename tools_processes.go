...
// notifyProcessDescription documents run_process's notify parameter. What it
// does lives in codemcp agent (see watchForProcessCompletion in agent.go),
// not here: this server has no conversation to tell anything, so the
// parameter is simply accepted and otherwise ignored by a caller with no use
// for it.
const notifyProcessDescription = "Only meaningful in a client that keeps its own conversation, such as codemcp " +
	"agent. If true, once this process finishes on its own - not already caught by wait_seconds - the model is " +
	"told without having to poll check_process_id for it. Has no effect on a process that wait_seconds already " +
	"waited out to completion in this same call, since the result already says so."
...
func (s *Server) registerProcessTools() {
	...
	s.RegisterTool(Tool{
		...
		InputSchema: schema([]string{"command"}, map[string]any{
			...
			"notify": propDefault("boolean", notifyProcessDescription, false),
		}),
