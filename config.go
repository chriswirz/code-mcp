...
type WorkspaceConfig struct {
	...
	// PathSuggestions is how manycandidates a not-found error offers when a
	// tool is given a path that is not there. Zero switches the suggestions
	// off; an absent key means the default.
	PathSuggestions int `json:"path_suggestions,omitempty"`

	// ExactPathNotRequired makes the file tools forgiving about where a file
	// is: a path that does not exist, whose file name matches exactly one file
	// in the workspace, is taken to mean that file, and the operation is
	// performed on it. Off by default - a correction that ends in a write is a
	// decision an operator should make deliberately - and either way the error
	// names the file that was meant.
	ExactPathNotRequired bool `json:"exact_path_not_required,omitempty"`

	// MaxLineEndingFiles caps how many files one fix_line_endings call may
	// rewrite. The tool counts the affected files before it writes anything
	// and refuses the whole operation when the count is over this, so a scope
	// that was wider than intended fails loudly instead of rewriting a tree.
	MaxLineEndingFiles int `json:"max_line_ending_files,omitempty"`

	// LineEndings makes the workspace speak one convention regardless of what
	// is on disk: "lf", "crlf" or "native" (this machine's) normalise every
	// read, every write and every find-and-replace comparison, so an edit
	// written on Windows against a file checked out with LF cannot half-convert
	// it. "preserve" (the default, and what an empty value means) leaves each
	// file exactly as it is and reconciles line endings per edit instead.
	LineEndings string `json:"line_endings,omitempty"`

	// RollbackDepth is how many earlier states of each file are kept in
	// memory for the rollback tool. Every write adds one; zero turns it off.
	RollbackDepth int `json:"rollback_depth"`

	// NoteAbsolutePaths appends, to every tool call that took a workspace-
	// relative path, a note naming the absolute path it actually resolved
	// to. Off by default: the workspace-relative path already returned is
	// what a model normally wants, and the absolute one is only useful in
	// setups where something outside this server - a shell, another tool -
	// needs to be told exactly where a file lives on disk.
	NoteAbsolutePaths bool `json:"note_absolute_paths,omitempty"`
}
...
type CommandConfig struct {
	...
	// Background runs the command through the process registry instead of
	// blocking: the tool returns a process id at once, and the output is read
	// back with check_process_id. For dev servers, watchers and anything else
...
func DefaultConfig() Config {
	return Config{
		...
		Workspace: WorkspaceConfig{
			...
			PathSuggestions:    defaultPathSuggestions,
			RollbackDepth:      defaultRollbackDepth,
		},
		Git: GitConfig{
			Enabled:       true,
			GitPath:       "git",
...
func LoadConfig(path string, explicit bool) (Config, error) {
	...
	data, err = dropUnknownSections(data)
	if err != nil {
...
// dropUnknownSections removes top-level keys that name no section of Config,
// and underscore-prefixed keys at any depth. JSON has no comment syntax, so
// an "_note" key is the usual way to annotate a config file, and rejecting the
// file over one would be unhelpful. Other unknown keys *inside* a known
// section are still an error, since there a stray key is far more likely to be
// a misspelt setting that would otherwise be silently ignored.
func dropUnknownSections(data []byte) ([]byte, error) {
	var doc map[string]json.RawMessage
	...
	for name, raw := range doc {
		...
		stripped, changed, err := dropCommentKeys(raw)
		if err != nil {
...
// dropCommentKeys removes every underscore-prefixed key from an arbitrary JSON
// value, descending through objects and arrays so a comment can sit beside any
// setting - next to one command in the commands array, say. It reports whether
// anything was removed, so an untouched document is decoded as it was written.
func dropCommentKeys(raw json.RawMessage) (json.RawMessage, bool, error) {
	switch first := firstToken(raw); first {
	case '{':
		...
		for name, value := range obj {
			...
			stripped, inner, err := dropCommentKeys(value)
			if err != nil {
				return raw, false, fmt.Errorf("%s: %w", name, err)
			}
			if inner {
				obj[name] = stripped
				changed = true
...
// firstToken is the first non-space byte of a JSON value, which is enough to
// tell an object or array from a scalar.
...
func (c *Config) Normalize(workspace string) error {
	...
	if c.Workspace.MaxLineEndingFiles <= 0 {
		c.Workspace.MaxLineEndingFiles = 500
	}
	if c.Workspace.RollbackDepth < 0 {
		c.Workspace.RollbackDepth = 0
	}
	if c.Workspace.PathSuggestions < 0 {
		// Negative is meaningless; zero is the honest way to switch them off,
		// and an absent key keeps the default this started from.
		c.Workspace.PathSuggestions = 0
	}
	switch strings.ToLower(strings.TrimSpace(c.Workspace.LineEndings)) {
	case "", "preserve", "keep", "as-is":
		c.Workspace.LineEndings = "preserve"
	case "lf", "unix":
		c.Workspace.LineEndings = "lf"
	case "crlf", "windows", "dos":
		c.Workspace.LineEndings = "crlf"
	...
	default:
		return fmt.Errorf("workspace.line_endings: want %q, %q, %q or %q, got %q",
			"preserve", "lf", "crlf", "native", c.Workspace.LineEndings)
	}
...
const ExampleConfig = `{
  ...
  "commands": [
    ...
    {
      "name": "serve",
      "description": "Run the development server in the background.",
  ...
  "agent": {
    "_note": "Used by 'codemcp agent'. Secrets can be inline, in a *_env variable, or a ${NAME} reference.",
    "default_model": "local",
    "system_prompt": "",
    "max_turns": 50,
    ...
    "always_allow": [],
    "models": [
      { "name": "local", "base_url": "10.0.0.200:11434/v1", "model": "", "style": "auto" },
      {
        "name": "gateway",
    ...
    "mcp_servers": [
      {
        ...
        "auth": { "type": "bearer", "token_env": "REMOTE_MCP_TOKEN" }
      },
      {
        "name": "vendor",
        "disabled": true,
        "url": "https://api.vendor.example.com/mcp",
        "_note": "Not every service takes Authorization: Bearer. Secrets may be written inline.",
        "auth": { "type": "header", "name": "X-API-Key", "token": "" },
        "headers": { "X-Tenant": "acme" }
      },
      { "name": "fs", "disabled": true, "command": "npx", "args": ["-y", "@modelcontextprotocol/server-filesystem", "."] }
    ],
    "openapi_servers": [
      {
        "name": "petstore",
        ...
        "auth": { "type": "header", "name": "api_key", "token_env": "PETSTORE_KEY" }
      }
    ]
  }
}
