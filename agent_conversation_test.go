...
func TestCommandSaveSessionWritesJSONLWithDefaultName(t *testing.T) {
	a, out := newConversationTestAgent(t, "irrelevant")
	a.history = []chatMessage{
		{Role: "user", Content: "do X"},
		{Role: "assistant", Content: "", ToolCalls: []toolCall{{ID: "c1", Name: "echo", Arguments: json.RawMessage(`{"text":"hi"}`)}}},
	...
	a.command("/savesession")
	msg := out.String()
	if !strings.Contains(msg, "saved 3 messages") || !strings.Contains(msg, "/loadsession") {
		t.Fatalf("output = %q", msg)
	}
	...
	var found string
	for _, e := range entries {
		if strings.HasPrefix(e.Name(), "codemcp-session-") {
			found = e.Name()
		}
	}
	if found == "" {
		t.Fatal("no codemcp-session- file was written")
	}
}

func TestCommandSaveSessionOnEmptyHistoryReportsError(t *testing.T) {
	a, out := newConversationTestAgent(t, "irrelevant")
	a.command("/savesession")
	if !strings.Contains(out.String(), "error:") {
		t.Errorf("output = %q, want an error for nothing to save", out.String())
...
// TestCommandLoadSessionRoundTrips is the point of the pair: what /savesession
// wrote, a later agent - a fresh process, a switched model - reads back with
// /loadsession exactly as it was, tool calls included.
func TestCommandLoadSessionRoundTrips(t *testing.T) {
	saver, out := newConversationTestAgent(t, "irrelevant")
	saver.history = []chatMessage{
		{Role: "user", Content: "do X"},
		{Role: "assistant", Content: "did Y", ToolCalls: []toolCall{{ID: "c1", Name: "echo", Arguments: json.RawMessage(`{"text":"hi"}`)}}},
		{Role: "tool", ToolCallID: "c1", ToolName: "echo", Content: "hi"},
	}
	saver.command("/savesession")
	if !strings.Contains(out.String(), "saved 3 messages") {
		t.Fatalf("save output = %q", out.String())
	}

	// A separate agent over the same workspace root, standing in for the
	// restarted process or the switched model /loadsession is meant for.
	loader, loaderOut := newConversationTestAgent(t, "irrelevant")
	loader.cfg.Workspace.Root = saver.cfg.Workspace.Root

	loader.command("/loadsession")
	if !strings.Contains(loaderOut.String(), "loaded 3 messages") {
		t.Fatalf("output = %q", loaderOut.String())
	}
	if len(loader.history) != 3 {
		t.Fatalf("history = %+v, want the 3 saved messages", loader.history)
	}
	if loader.history[0].Content != "do X" {
		t.Errorf("history[0] = %+v", loader.history[0])
	}
	if len(loader.history[1].ToolCalls) != 1 || loader.history[1].ToolCalls[0].Name != "echo" {
		t.Errorf("history[1] tool calls = %+v", loader.history[1].ToolCalls)
	}
	if loader.history[2].Role != "tool" || loader.history[2].ToolCallID != "c1" {
		t.Errorf("history[2] = %+v", loader.history[2])
	}
}

// TestCommandLoadSessionReplacesExistingHistory: unlike /loadsummary, which
// prepends, /loadsession stands in for a.history outright - it is meant to
// pick a session back up, not merge two of them.
func TestCommandLoadSessionReplacesExistingHistory(t *testing.T) {
	a, _ := newConversationTestAgent(t, "irrelevant")
	root := a.cfg.Workspace.Root
	saved := []transcriptLine{{Role: "user", Content: "from the saved session"}}
	writeJSONL(t, filepath.Join(root, "codemcp-session-20260101-120000.jsonl"), saved)

	a.history = []chatMessage{{Role: "user", Content: "still here"}}
	a.command("/loadsession")

	if len(a.history) != 1 || a.history[0].Content != "from the saved session" {
		t.Errorf("history = %+v, want it replaced by the loaded session", a.history)
	}
}

func TestCommandLoadSessionDefaultsToLatest(t *testing.T) {
	a, out := newConversationTestAgent(t, "irrelevant")
	root := a.cfg.Workspace.Root
	writeJSONL(t, filepath.Join(root, "codemcp-session-20260101-120000.jsonl"),
		[]transcriptLine{{Role: "user", Content: "older session"}})
	writeJSONL(t, filepath.Join(root, "codemcp-session-20260914-153000.jsonl"),
		[]transcriptLine{{Role: "user", Content: "the latest session"}})

	a.command("/loadsession")
	if !strings.Contains(out.String(), "loaded 1 messages") {
		t.Fatalf("output = %q", out.String())
	}
	if len(a.history) != 1 || a.history[0].Content != "the latest session" {
		t.Errorf("history = %+v, want the latest session's message", a.history)
	}
}

func TestCommandLoadSessionWithExplicitPath(t *testing.T) {
	a, out := newConversationTestAgent(t, "irrelevant")
	writeJSONL(t, filepath.Join(a.cfg.Workspace.Root, "my-session.jsonl"),
		[]transcriptLine{{Role: "user", Content: "custom session"}})

	a.command("/loadsession my-session.jsonl")
	if !strings.Contains(out.String(), "loaded 1 messages") {
		t.Fatalf("output = %q", out.String())
	}
	if len(a.history) != 1 || a.history[0].Content != "custom session" {
		t.Errorf("history = %+v", a.history)
	}
}

func TestCommandLoadSessionNoneFoundReportsError(t *testing.T) {
	a, out := newConversationTestAgent(t, "irrelevant")
	a.command("/loadsession")
	if !strings.Contains(out.String(), "error:") {
...
// writeJSONL writes lines as a JSONL file, for tests that need a session file
// on disk without going through /savesession to create it.
...
func TestPrintBannerMentionsSavedSession(t *testing.T) {
	a, out := newConversationTestAgent(t, "irrelevant")
	writeJSONL(t, filepath.Join(a.cfg.Workspace.Root, "codemcp-session-20260914-153000.jsonl"),
		[]transcriptLine{{Role: "user", Content: "x"}})

	a.printBanner()
	if !strings.Contains(out.String(), "/loadsession to resume") {
		t.Errorf("banner = %q, want a hint about the saved session", out.String())
	}
}
