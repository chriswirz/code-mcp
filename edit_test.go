...
func TestApplyEdit(t *testing.T) {
	...
	t.Run("single occurrence", func(t *testing.T) {
		res, err := applyEdit(content, editOp{Path: "f", OldString: "alpha", NewString: replacement("ALPHA")}, true)
		if err != nil {
	...
	t.Run("ambiguous anchor is refused", func(t *testing.T) {
		if _, err := applyEdit(content, editOp{Path: "f", OldString: "beta", NewString: replacement("B")}, true); err == nil {
			t.Fatal("expected an error for an anchor that appears twice")
		}
	})

	t.Run("replace_all", func(t *testing.T) {
		res, err := applyEdit(content, editOp{Path: "f", OldString: "beta", NewString: replacement("B"), ReplaceAll: true}, true)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if res.Count != 2 {
	...
	t.Run("missing anchor", func(t *testing.T) {
		if _, err := applyEdit(content, editOp{Path: "f", OldString: "delta", NewString: replacement("D")}, true); err == nil {
			t.Fatal("expected an error for an anchor that does not appear")
		}
	})

	t.Run("empty anchor", func(t *testing.T) {
		if _, err := applyEdit(content, editOp{Path: "f", NewString: replacement("x")}, true); err == nil {
...
func TestApplyEditCamelCaseAliases(t *testing.T) {
	...
	t.Run("oldText and newText stand in", func(t *testing.T) {
		res, err := applyEdit(content, editOp{Path: "f", OldText: "alpha", NewText: replacement("ALPHA")}, true)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if res.Content != "ALPHA\nbeta\n" {
	...
	t.Run("snake_case wins when both are given", func(t *testing.T) {
		op := editOp{Path: "f", OldString: "alpha", NewString: replacement("A"), OldText: "beta", NewText: replacement("B")}
	...
	t.Run("aliases still normalise line endings", func(t *testing.T) {
		res, err := applyEdit("alpha\r\nbeta\r\n", editOp{Path: "f", OldText: "alpha\nbeta", NewText: replacement("alpha\nBETA")}, true)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if res.Content != "alpha\r\nBETA\r\n" {
	...
	t.Run("anchor resolution", func(t *testing.T) {
		cases := []struct {
			...
			{"snake_case only", editOp{OldString: "a", NewString: replacement("b")}, "a", "b"},
			{"camelCase only", editOp{OldText: "a", NewText: replacement("b")}, "a", "b"},
			{"snake_case wins", editOp{OldString: "a", NewString: replacement("b"), OldText: "x", NewText: replacement("y")}, "a", "b"},
			{"mixed spellings", editOp{OldString: "a", NewText: replacement("b")}, "a", "b"},
			{"neither", editOp{}, "", ""},
		}
		for _, tc := range cases {
			gotOld, gotNew, _ := tc.op.anchor()
	...
	t.Run("deleting text with an empty replacement", func(t *testing.T) {
		// An explicit empty new_string means "delete", and must not fall
		// through to newText.
		res, err := applyEdit("keep\ndrop\n", editOp{
			Path: "f", OldString: "drop\n", NewString: replacement(""), NewText: replacement("SHOULD NOT APPEAR"),
		}, true)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("an absent replacement is refused", func(t *testing.T) {
		// The difference this draws is the point: an empty replacement deletes
		// on purpose, an absent one used to delete by accident.
		_, err := applyEdit("keep\ndrop\n", editOp{Path: "f", OldString: "drop\n"}, true)
		if err == nil {
			t.Fatal("an edit with no replacement should be refused, not applied as a deletion")
		}
